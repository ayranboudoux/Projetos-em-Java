# Projetos em Java — Backend

Este repositório reúne meus projetos desenvolvidos em **Java**, desde os primeiros estudos até aplicações backend mais completas, com foco em **boas práticas** e **organização de código**.

O objetivo é documentar minha evolução como desenvolvedor backend e servir como **portfólio**.

---

## Conteúdo do Repositório

Os projetos estão organizados de forma progressiva, acompanhando meu aprendizado:

```text
📁 java-projects
┣ 📂 fundamentos
┃ ┗ Conceitos básicos da linguagem Java
┣ 📂 oop
┃ ┗ Programação Orientada a Objetos
┣ 📂 collections
┃ ┗ List, Set, Map e Streams
┣ 📂 jdbc
┃ ┗ Integração com banco de dados
┣ 📂 spring-boot
┃ ┗ APIs REST com Spring Boot
┣ 📂 desafios
┃ ┗ Exercícios e desafios práticos
```

---

## Tecnologias e Ferramentas

- Java
- Spring Boot
- Maven / Gradle
- JDBC
- JPA / Hibernate
- APIs REST
- Banco de Dados (MySQL / PostgreSQL)
- Git e GitHub

---
